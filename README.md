...
**Client-side document signing, cryptographic fingerprinting, and leak attribution — with Web3 wallet-gated delivery.**

ShadowSign is a fully self-hostable, single-file HTML application that lets you stamp documents with invisible cryptographic watermarks, generate signed delivery packages for specific Ethereum wallet addresses, and verify attribution if a document is ever leaked. All cryptography runs in the browser. No backend is required for signing or verification — only for the optional Web3 encrypted delivery relay.
